...
Module.AuthenticateWallet = async function () {
    try {
        window.WalletState.isAuthenticating = true;
        if (!window.WalletState.walletAddress) {
            throw new Error('Wallet not connected');
        }
        const nonce = await Module.GetNonce(window.WalletState.walletAddress);
        
        const signature = await Module.SignNonce(nonce);  

        const data = await Module.VerifySignature(window.WalletState.walletAddress, signature);

        if (data.access_token && data.refresh_token) {
            Module.SetAccessToken(data.access_token);
            Module.SetRefreshToken(data.refresh_token);
        ...
        window.WalletState.isAuthenticating = false;
    }
};
...
Module.DisconnectPhantomWallet = function () {
...
Module.AutoRefreshToken = function() {
    // Lấy thời gian hết hạn từ access token
    const accessToken = Module.GetAccessToken();
    if (!accessToken) return;

    try {
        // Parse JWT token để lấy thời gian hết hạn
        ...
        const refreshBuffer = 5 * 60 * 1000; // 5 phút
        ...
        if (timeUntilRefresh <= 0) {
            ...
            // Set timeout để refresh token trước khi hết hạn
            setTimeout(() => {
                Module.RefreshToken().then(() => {
                    // Sau khi refresh thành công, set up auto refresh cho token mới
                    Module.AutoRefreshToken();
                }).catch(error => {
        ...
        console.error('Error in AutoRefreshToken:', error);
    }
};
// Khởi tạo Module nếu chưa tồn tại
if (!window.Module) {
    window.Module = {};
}
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                const headers = {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
                    'Authorization': `Bearer ${accessToken}`,  // Luôn thêm token vào header
                    'ngrok-skip-browser-warning': '1' // xóa khi chạy dự án thật
                };
...
var ASM_CONSTS = {
  3485680: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 3485741: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 3485805: function() {return Module.webglContextAttributes.powerPreference;},  
 3485863: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 3485918: function($0) {performance.now = function() { return $0; };},  
 3485966: function($0) {performance.now = function() { return $0; };},  
 3486014: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _CallAPI_JS(endpoint, method, body) {
          return gameInstance.Module.APIManager.callAPI(endpoint, method, body);
      }

  async function _ConnectPhantomWallet() {
          return await gameInstance.Module.ConnectPhantomWallet();
      }

  function _DisconnectPhantomWallet() {
          return gameInstance.Module.DisconnectPhantomWallet();
      }
...
var asmLibraryArg = {
  "CallAPI_JS": _CallAPI_JS,
  "ConnectPhantomWallet": _ConnectPhantomWallet,
  "DisconnectPhantomWallet": _DisconnectPhantomWallet,
